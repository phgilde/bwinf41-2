    ...
    class SubTourCutGenerator(ConstrsGenerator):
        ...
        def generate_constrs(self, model: Model, depth: int = 0, npass: int = 0):
            ...
            try:
                ...
                cut = xsum(xf[edge(u, v)] for u, v, _ in cycle) <= len(cycle) - 1
                model += cut
            ...
            if self.use_johnson:
                ...
                node1, node2 = max(
                    product(V_, V_),
                    key=lambda e: nx.path_weight(G, D[e[0]].get(e[1], [0]), "capacity")
                    if e[0] != e[1]
                    else 0,
                )
            else:
                node1, node2 = max(
                    filter(
                        lambda e: e[0] in G.nodes and e[1] in G.nodes, product(V_, V_)
                    ),
                    key=lambda e: c[e[0]][e[1]] if e[0] != e[1] else 0,
                )
            ...
            if cut_value < 1 - 1e-6:
                cut = (
                    xsum(xf[edge(u, v)] for u, v in G.subgraph(TS).edges)
                    <= len(G.subgraph(TS).edges) - 1
                )
                model += cut
